...
class WithdrawController extends Controller {
    ...
    public function go(Request $r) {
        ...
        if (\Cache::has('action.user.' . $user->id)) return response(['success' => false, 'mess' => 'Подождите 5 сек.']);
        \Cache::put('action.user.' . $user->id, '', 5);
        ...
        if ($user_deps < $dep_withdraw && $user->admin != 3) {
            return response(['success' => false, 'mess' => 'Для вывода требуется минимальная сумма пополнений за 7 дней - '.$dep_withdraw.' (У вас ' . $user_deps . 'р)']);
        }
