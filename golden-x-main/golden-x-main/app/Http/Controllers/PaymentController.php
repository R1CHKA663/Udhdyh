...
class PaymentController extends Controller {
	...
	public function resultRubpay(Request $r) {
		...
		$hash = md5("1184" . $r->order_id . $r->payment_id . $r->amount . $r->currency . $r->status . "fccfcd2446121673f5b787ec11bfa582");
	...
	public function go(Request $r) {
		if(\Cache::has('action#deposit.user.' . $this->user->id)) return response(['success' => false, 'mess' => 'Подождите перед предыдущим действием 30 секунд!']);
    \Cache::put('action#deposit.user.' . $this->user->id, '', 30);
		$sum = $r->sum;
		$system = $r->system;
		$promo = $r->promo;
		...
		if ($psDep == 6) {
			$payload = http_build_query([
				...
				'amount' => $sum,
				'order_id' => $unique_id,
				'sign' => md5("fccfcd2446121673f5b787ec11bfa582" . "1184" . $unique_id . $sum . "1" . "fccfcd2446121673f5b787ec11bfa582"),
				'payment_method' => 1
			]);
