...
class Controller extends BaseController {
    ...
    protected $settings;
    protected $user;
    protected $OPERATOR_ID = 40084;
    protected $YT_OPERATOR_ID = 40085;

    public function __construct() {
        $this->redis = FacadesRedis::connection();
        $this->settings = Setting::first();

        $this->OPERATOR_ID = $this->settings->b2b_operator_id;
		$this->YT_OPERATOR_ID = $this->settings->b2b_yt_operator_id;
    ...
    public function changePhoto(Request $request) {
        ...
        if(Auth::user()->change_avatar) return ['error' => true, 'msg' => 'Вы уже меняли аватар'];

        $image = $request->photo;
        $image_filename = Str::random(8) .'_'. $image->getClientOriginalName();

        $image = Image::make($image->getRealPath());
        $image->save(public_path('uploads/' .$image_filename));

        User::where('id', Auth::id())->update([
            'avatar' => '/uploads/' . $image_filename,
            'change_avatar' => 1
        ]);
    ...
    public function changeName(Request $request) {
        if(Auth::guest()) return;
        if(Auth::user()->change_name) return ['error' => true, 'msg' => 'Вы уже меняли ник'];
        if(strlen(trim($request->name)) < 5 || strlen(trim($request->name)) > 15) return ['error' => true, 'msg' => 'Длина ника от 5 до 15 символов'];
    ...
    public function promoAct(Request $r) {
        ...
        if ($user->balance >= 5) {
            return response(['success' => false, 'mess' => 'Баланс больше 5!']);
        }
        if (\Cache::has('action.user.' . $user->id)) return response(['success' => false, 'mess' => 'Подождите перед предыдущим действием!']);
        \Cache::put('action.user.' . $user->id, '', 1);
    ...
    public function bonusGetVk(Request $request) {
        ...
        if ($data['response'] != '1') {
            return response(['success' => false, 'link' => $setting->group_link, 'mess' => 'Подпишитесь на нашу группу!']);
        }
    ...
    public function bonusGet(Request $request) {
        ...
        if ($f_time < 43200) {
            return response(['success' => false, 'mess' => "Ожидайте еще {$w__time}"]);
        }
