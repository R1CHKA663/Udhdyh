...
class AdminController extends Controller
{
    ...
    public function getBalance(Request $r) {
        ...
        ksort($data);
        $sign = hash_hmac('sha256', implode('|', $data), "");
        $data['signature'] = $sign;
        ...
        */
        ...
        $payload = http_build_query([
            'project_id' => 1184,
            'sign' => md5("fccfcd2446121673f5b787ec11bfa582" . "1184" . "fccfcd2446121673f5b787ec11bfa582"),
        ]);

        $response = json_decode(file_get_contents("https://rubpay.io/api/project/balance?" . $payload));

        return response()->json(['rubpay_bal' => $response->balance[0]->amount, 'fk_bal' => 0.00]);
    }
    ...
    public function createPromo(Request $r){
        ...
        $count_promo = Promo::where('user_id', $this->user->id)->where('created_at', '>=', Carbon::now()->subDays(1)->toDateTimeString())->count();
        ...
        if(\Auth::user()->admin == 2 && $sum > 15) return response()->json(['message' => 'Максимальная сумма 15'], 401);
        if(\Auth::user()->admin == 2 && $count_promo > 10) return response()->json(['message' => 'Максимальное количество 10 прмокодов в день!'], 401);
