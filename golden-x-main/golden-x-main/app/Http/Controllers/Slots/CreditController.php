...
class CreditController extends Controller
{
    public function credit($request)
    {
        ...
        $error = "ok";
        try {
            $credit_amount = $request->data->credit_amount > 0 ? $request->data->credit_amount : 0;
            $amount = $this->userUpdateAmount($request->data->user_id, $credit_amount, $request->data->currency);
        } catch (\Exception $e) {
            $error = $e->getMessage();
        }
        ...
        $response->answer->user_nickname = $user->name;
        ...
        $response->answer->error_description = $error;
