	...
	function getWithdraws(type = '', mult) {
		...
		$.post('/admin/withdraws/all',{_token: csrf_token, type, mult}).then(e=>{
			...
			e.withdraws.forEach((e)=>{
				buttons = '<button class="btn-dep w-100" onclick="withdraw_menu('+e.status+', '+e.id+')"><i class="fa fa-list" style="font-size:16px"></i></button>';
				system = '';
				if(e.status  == 1){
					...
					buttons += '<button class="btn-dep w-100" onclick="disable(this);updateWithdraw(5, this, '+e.id+')">FK</button><button class="btn-dep w-100" onclick="updateWithdraw(2, this, '+e.id+')">Отмена</button>';
				}else if(e.status == 2){
